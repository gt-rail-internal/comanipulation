function[expData, expSigma] = UOLA_predict(model_dir,observation, writeDir)
%%%% ,writeDir, varWriteDir, sampledWriteDir, sampledVarWriteDir
%%%% Version 1.1.0
...
if num_total_traj>=1
    num_pred = 100; %%Number of predictions needed
...
%% Second layer part trajectory classification - Joint                
    ...
    if traj_len < traj_len_old_avg
        ...
        expData = expData';
        expSigma = permute(expSigma, [3, 1, 2]);
        expData = smooth_predict_traj( traj_joint,expData );%%%%%%%%only predicted trajectory
        expData(:,1) = [];
        %%%csvwrite(writeDir,expData);
        %%% csvwrite(varWriteDir, expSigma);
        %%%subsampledMeans = expData(1:10:end,:); 
        %%% subsampledVars = expSigma(1:10:end,:);
        %%csvwrite(writeDir,subsampledMeans);
        %%% csvwrite(sampledVarWriteDir, subsampledVars); 
    else
        ...
        expData = [];
        expData(1,:) = traj_len:1:traj_len+num_pred;
        [expData(2:num_feature_joint,:), expSigma] = GMR( ...
            GMM_lib_joint{1,predict_gmm_p}{predict_gmm_p_joint}.prior, ...
            GMM_lib_joint{1,predict_gmm_p}{predict_gmm_p_joint}.mu, ...
            GMM_lib_joint{1,predict_gmm_p}{predict_gmm_p_joint}.sigma, ...
