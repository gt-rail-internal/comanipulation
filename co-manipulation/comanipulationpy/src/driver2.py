...
def multiple_locations(trajectory, joint_start, joint_targets, execute_comanipulation, execute_baseline, plot_baselines, num_baselines, num_metrics):
    ...
    for locIndex, joint_target in enumerate(joint_targets):
        all_comanipulation_metrics[:, locIndex] = comanipulationFramework.setup_test(joint_start, joint_target, traj_num=trajectory, execute=execute_comanipulation)
        baselineTest = Test(robot,joint_start, joint_target, traj_num=trajectory, execute=execute_baseline)
        all_baseline_metrics[:, :, locIndex] = baselineTest.run_all_baselines()
        metrics_to_csv(trajectory, all_comanipulation_metrics[:, locIndex], all_baseline_metrics[:, :, trajIndex])
...
if __name__ == "__main__":
    ...
    joint_targets = [[4.871800476914653, 1.895875884203746, 4.728695515739245, 1.2668175273349631, 4.713923493804794, 4.641769937759059, 5.034508434241916]]
    #trajectories = [120]
    trajectory = 120
    ...
    multiple_locations(trajectory, joint_start, joint_targets, execute_comanipulation, execute_baseline, plot_baselines, num_baselines, num_metrics)  
