...
void RegisterMakers() {
  ...
  TermInfo::RegisterMaker("test_cost", &TestCostInfo::create);
  // Co-manipulation costs
  TermInfo::RegisterMaker("distance_cost", &DistanceCostInfo::create);
  TermInfo::RegisterMaker("visibility_cost", &VisibilityCostInfo::create);
  TermInfo::RegisterMaker("velocity_cost", &VelocityCostInfo::create);
  TermInfo::RegisterMaker("legibility_cost", &LegibilityCostInfo::create);
  // Co-manipulation additional costs
  TermInfo::RegisterMaker("regularize_cost", &RegularizeCostInfo::create);
  TermInfo::RegisterMaker("smoothing_cost", &SmoothingCostInfo::create);
...
// TestCostInfo TODO
// For now, this just copies JointVelCostInfo for testing purposes
void TestCostInfo::fromJson(const Value& v) {
  FAIL_IF_FALSE(v.isMember("params"));
  const Value& params = v["params"];
  childFromJson(params, coeffs,"coeffs");
  int n_dof = gPCI->rad->GetDOF();
...
void TestCostInfo::hatch(TrajOptProb& prob) {
  prob.addCost(CostPtr(new TestCost(prob.GetVars(), toVectorXd(coeffs))));
  prob.getCosts().back()->setName(name);
}
...
// Cost info for distance
void DistanceCostInfo::fromJson(const Value& v) {
  int n_steps = gPCI->basic_info.n_steps;
  FAIL_IF_FALSE(v.isMember("params"));
  const Value& params = v["params"];
  childFromJson(params, human_poses_mean_v,"human_poses_mean");
  childFromJson(params, human_poses_var_v,"human_poses_var");
  childFromJson(params, n_human_joints, "n_human_joints");
  childFromJson(params, coeffs_v, "coeffs");
  coeffs = VectorXd::Zero(n_steps);

  int mean_v_size = n_steps * n_human_joints * 3;
  int var_v_size = n_steps * n_human_joints * 9;
  // Check human pose mean and var sizes
  if (human_poses_mean_v.size() != mean_v_size) {
    PRINT_AND_THROW(boost::format("incorrect number of human pose means. Expected %i, got %i") % mean_v_size % human_poses_mean_v.size());
  }
  if (human_poses_var_v.size() != var_v_size) {
    PRINT_AND_THROW(boost::format("incorrect number of human pose vars. Expected %i, got %i") % var_v_size % human_poses_var_v.size());
  }
  if (coeffs_v.size() != n_steps) {
    PRINT_AND_THROW(boost::format("incorrect number of coeffs. Expected %i, got %i") % n_steps % coeffs_v.size());
  }

  // Create vector of all robot links
  ...
  for (int i = 0; i < coeffs_v.size(); i++) {
    coeffs(i) = coeffs_v.at(i);
  }

  // Create vectors of human pose means and variances
  for (int i = 0; i < n_steps; i++) {
    for (int j = 0; j < n_human_joints; j++) {
      Vector3d pose_mean;
      for (int k = 0; k < 3; k++) {
        pose_mean(k) = human_poses_mean_v.at(i * n_human_joints * 3 + j * 3 + k);
      }
      human_poses_mean.push_back(pose_mean);
      Matrix3d pose_var;
      for (int k = 0; k < 9; k++) {
        pose_var(k) = human_poses_var_v.at(i * n_human_joints * 9 + j * 9 + k);
      }
      human_poses_var.push_back(pose_var);
    }
  }

  const char* all_fields[] = {"human_poses_mean", "human_poses_var", "n_human_joints", "coeffs", "links"};
  ensure_only_members(params, all_fields, sizeof(all_fields)/sizeof(char*));

}

void DistanceCostInfo::hatch(TrajOptProb& prob) {
  VectorOfVectorPtr f(new DistanceCostCalculator(human_poses_mean, human_poses_var, links, n_human_joints, prob.GetRAD()));
  if (term_type == TT_COST) {
    // VectorXd coeffs_(1);
    // coeffs_ << coeffs;
    prob.addCost(CostPtr(new CostFromErrFunc(f, prob.GetVarsAsVec(), coeffs, ABS, name)));
  }

  // prob.GetPlotter()->Add(PlotterPtr(new DistanceCostPlotter(f, prob.GetVarsAsVec())));
  // prob.GetPlotter()->AddLink(link);

}

// Cost info for velocity
void VelocityCostInfo::fromJson(const Value& v) {
  int n_steps = gPCI->basic_info.n_steps;
  FAIL_IF_FALSE(v.isMember("params"));
  const Value& params = v["params"];
  childFromJson(params, human_poses_mean_v,"human_poses_mean");
...
void VelocityCostInfo::hatch(TrajOptProb& prob) {
  VectorOfVectorPtr f(new VelocityCostCalculator(human_poses_mean, human_poses_var, links, n_human_joints, prob.GetRAD()));
  if (term_type == TT_COST) {
    // VectorXd coeffs_(1);
    // coeffs_ << coeffs;
    prob.addCost(CostPtr(new CostFromErrFunc(f, prob.GetVarsAsVec(), coeffs, ABS, name)));
...
// Cost info for visibility
void VisibilityCostInfo::fromJson(const Value& v) {
  int n_steps = gPCI->basic_info.n_steps;
  FAIL_IF_FALSE(v.isMember("params"));
  const Value& params = v["params"];
  childFromJson(params, head_pos_mean_v,"head_pos_mean");
  childFromJson(params, head_pos_var_v,"head_pos_var");
  // childFromJson(params, head_pos_mean_v,"human_poses_mean");
  // childFromJson(params, head_pos_var_v,"human_poses_var");
  childFromJson(params, obj_pos,"obj_pos");
  // childFromJson(params, coeffs,"coeffs");
  ...
  // Check human pose mean and var sizes
  // if (head_pos_mean.size() != n_steps) {
  //   PRINT_AND_THROW(boost::format("incorrect number of human head pos means. Expected %i, got %i") % n_steps % head_pos_mean.size());
  // }
  // if (head_pos_var.size() != n_steps) {
  //   PRINT_AND_THROW(boost::format("incorrect number of human head pos vars. Expected %i, got %i") % n_steps % head_pos_var.size());
  // }
  if (coeffs_v.size() != n_steps) {
    PRINT_AND_THROW(boost::format("incorrect number of coeffs. Expected %i, got %i") % n_steps % coeffs_v.size());
  }
  ...
  for (int i = 0; i < n_steps; i++) {
    ...
    Vector3d pose_var;
    for (int k = 0; k < 3; k++) {
      pose_mean(k) = head_pos_mean_v.at(i * 3 + k);
      pose_var(k) = head_pos_var_v.at(i * 3 + k);
    }
    head_pos_mean.push_back(pose_mean);
    head_pos_var.push_back(pose_var);
  }

  const char* all_fields[] = {"head_pos_mean", "head_pos_var", "obj_pos", "coeffs", "link"};
  ensure_only_members(params, all_fields, sizeof(all_fields)/sizeof(char*));

}

void VisibilityCostInfo::hatch(TrajOptProb& prob) {
  VectorOfVectorPtr f(new VisibilityCostCalculator(head_pos_mean, head_pos_var, obj_pos, prob.GetRAD(), link));
  if (term_type == TT_COST) {
    // VectorXd coeffs_(1);
    // coeffs_ << coeffs;
    prob.addCost(CostPtr(new CostFromErrFunc(f, prob.GetVarsAsVec(), coeffs, ABS, name)));
  ...
  // prob.GetPlotter()->Add(PlotterPtr(new VisibilityCostCostPlotter(f, prob.GetVarsAsVec())));
  // prob.GetPlotter()->AddLink(link);
...
void LegibilityCostInfo::fromJson(const Value& v) {
  FAIL_IF_FALSE(v.isMember("params"));
  const Value& params = v["params"];
  childFromJson(params, coeffs, "coeffs");
...
// Cost info for regularization - distance in 1 timestep
void RegularizeCostInfo::fromJson(const Value& v) {
  int n_steps = gPCI->basic_info.n_steps;
  int coeffs_size = n_steps - 1;
  FAIL_IF_FALSE(v.isMember("params"));
  const Value& params = v["params"];
  
  childFromJson(params, coeffs_v, "coeffs");
  coeffs = VectorXd::Zero(coeffs_size);
  if (coeffs_v.size() != coeffs_size) {
    PRINT_AND_THROW(boost::format("incorrect number of coeffs. Expected %i, got %i") % coeffs_size % coeffs_v.size());
  }
  for (int i = 0; i < coeffs_v.size(); i++) {
    coeffs(i) = coeffs_v.at(i);
  }
...
void RegularizeCostInfo::hatch(TrajOptProb& prob) {
  VectorOfVectorPtr f(new RegularizeCostCalculator(prob.GetRAD(), link));
  if (term_type == TT_COST) {
    // VectorXd coeffs_(1);
    // coeffs_ << coeffs;
    prob.addCost(CostPtr(new CostFromErrFunc(f, prob.GetVarsAsVec(), coeffs, ABS, name)));
...
// Cost info for Smoothing
void SmoothingCostInfo::fromJson(const Value& v) {
  // int n_steps = gPCI->basic_info.n_steps;
  // int coeffs_size = n_steps - 1;
  FAIL_IF_FALSE(v.isMember("params"));
  const Value& params = v["params"];
  childFromJson(params, coeffs, "coeffs");
  childFromJson(params, type, "type");
  std::cout << "smoothing type: " << type << std::endl;
  ...
  const char* all_fields[] = {"coeffs", "type"};
  ensure_only_members(params, all_fields, sizeof(all_fields)/sizeof(char*));

}

void SmoothingCostInfo::hatch(TrajOptProb& prob) {
  VectorOfVectorPtr f;
  if (type == 1) {
    f = VectorOfVectorPtr(new SmoothingCostCalculator(prob.GetRAD()));
  } else {
    f = VectorOfVectorPtr(new SmoothingAccelCostCalculator(prob.GetRAD()));
  }
  if (term_type == TT_COST) {
    VectorXd coeffs_(1);
    coeffs_ << coeffs;
...
// Cost info for distance baseline
void DistanceBaselineCostInfo::fromJson(const Value& v) {
  FAIL_IF_FALSE(v.isMember("params"));
  const Value& params = v["params"];
  childFromJson(params, head_pos,"head_pos");
  childFromJson(params, torso_pos,"torso_pos");
...
void DistanceBaselineCostInfo::hatch(TrajOptProb& prob) {
  ...
  // prob.GetPlotter()->Add(PlotterPtr(new VisibilityBaselineCostPlotter(f, prob.GetVarsAsVec())));
  // prob.GetPlotter()->AddLink(link);

}


// Cost info for visibility baseline
void VisibilityBaselineCostInfo::fromJson(const Value& v) {
  FAIL_IF_FALSE(v.isMember("params"));
  const Value& params = v["params"];
  childFromJson(params, head_pos,"head_pos");
  childFromJson(params, obj_pos,"obj_pos");
  ...
  const char* all_fields[] = {"head_pos", "obj_pos", "coeffs", "link"};
  ensure_only_members(params, all_fields, sizeof(all_fields)/sizeof(char*));

}

void VisibilityBaselineCostInfo::hatch(TrajOptProb& prob) {
  VectorOfVectorPtr f(new VisibilityBaselineCostCalculator(head_pos, obj_pos, prob.GetRAD(), link));
  if (term_type == TT_COST) {
    VectorXd coeffs_(1);
    coeffs_ << coeffs;
    prob.addCost(CostPtr(new CostFromErrFunc(f, prob.GetVarsAsVec(), coeffs_, ABS, name)));
