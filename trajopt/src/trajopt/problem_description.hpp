...
public:
  ...
  VarVector GetVarsAsVec() {
    VarVector out;
    for(int i = 0; i < m_traj_vars.rows(); i++) {
      for(int j = 0; j < m_traj_vars.cols(); j++) {
        out.push_back(m_traj_vars.at(i, j));
      }
    }
    return out;
  }
...
// TestCostInfo TODO
struct TestCostInfo : public TermInfo, public MakesCost {
  DblVec coeffs;
  void fromJson(const Value& v);
  void hatch(TrajOptProb& prob);
  DEFINE_CREATE(TestCostInfo)
};
...
struct DistanceCostInfo : public TermInfo, public MakesCost, public MakesConstraint {
  // T * n vector of means and variances
  DblVec human_poses_mean_v;
  std::vector<Vector3d> human_poses_mean;
  DblVec human_poses_var_v;
  std::vector<Matrix3d> human_poses_var;
  int n_human_joints;
  DblVec coeffs_v;
  VectorXd coeffs;
  ...
  void fromJson(const Value& v);
  void hatch(TrajOptProb& prob);
  DEFINE_CREATE(DistanceCostInfo);
};

// Cost info for velocity
struct VelocityCostInfo : public TermInfo, public MakesCost, public MakesConstraint {
  // T * n vector of means and variances
  DblVec human_poses_mean_v;
  std::vector<Vector3d> human_poses_mean;
  DblVec human_poses_var_v;
  ...
  void fromJson(const Value& v);
  void hatch(TrajOptProb& prob);
  DEFINE_CREATE(VelocityCostInfo);
};


// Cost info for visibility
struct VisibilityCostInfo : public TermInfo, public MakesCost, public MakesConstraint {
  DblVec head_pos_mean_v;
  std::vector<Vector3d> head_pos_mean;
  DblVec head_pos_var_v;
  std::vector<Vector3d> head_pos_var;
  Vector3d obj_pos;
  DblVec coeffs_v;
  VectorXd coeffs;
  KinBody::LinkPtr link;
  void fromJson(const Value& v);
  void hatch(TrajOptProb& prob);
  DEFINE_CREATE(VisibilityCostInfo);
};


// Cost info for legibility
...
// Cost info for Regularization
struct RegularizeCostInfo : public TermInfo, public MakesCost, public MakesConstraint {
  DblVec coeffs_v;
  VectorXd coeffs;
  KinBody::LinkPtr link;
  void fromJson(const Value& v);
  void hatch(TrajOptProb& prob);
  DEFINE_CREATE(RegularizeCostInfo);
};

struct SmoothingCostInfo : public TermInfo, public MakesCost, public MakesConstraint {
  double coeffs;
  int type;
  void fromJson(const Value& v);
  void hatch(TrajOptProb& prob);
  DEFINE_CREATE(SmoothingCostInfo);
};
...
struct DistanceBaselineCostInfo : public TermInfo, public MakesCost, public MakesConstraint {
  Vector3d head_pos;
  Vector3d torso_pos;
  Vector3d feet_pos;
  double coeffs;
...
struct VisibilityBaselineCostInfo : public TermInfo, public MakesCost, public MakesConstraint {
  Vector3d head_pos;
  Vector3d obj_pos;
  double coeffs;
  KinBody::LinkPtr link;
