...
class TrajectoryPlanner:
    ...
    def solve_traj_save_plot_exec(self, init_joint, final_joint, coeffs={}, object_pos=[0, 0.2, 0.83],
                plot='', execute=False, save='', enable_estop=False, resume_safely=False):
        """
        ...
        if execute:
            ...
            self.scene.execute_full_trajectory(result.GetTraj(), self.full_rightarm_test_traj, len(
                self.obs_rightarm_test_traj) / 12, len(self.full_rightarm_test_traj) / 12, enable_estop=enable_estop, resume_safely=resume_safely)
        if plot != '':
